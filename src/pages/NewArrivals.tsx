  ...
  return (
    <div className="min-h-screen bg-cream pb-20">
      ...
      <div className="bg-stone-900 text-white py-16 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-warm-600 rounded-full blur-[100px] opacity-20 transform translate-x-1/2 -translate-y-1/2"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex items-center gap-3 mb-4 text-warm-400 font-medium tracking-wide uppercase text-sm">
            <Sparkles className="w-4 h-4" /> Just Landed
          </div>
          <h1 className="text-4xl md:text-6xl font-bold mb-6 leading-tight">
            Fresh from the <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-warm-400 to-amber-400">
              Design Studio
            </span>
          </h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {isLoading ? (
          <div className="flex justify-center h-64 items-center">
            <Loader2 className="h-10 w-10 animate-spin text-warm-600" />
          </div>
        ) : products.length === 0 ? (
                ...
                <Card key={product.id} className="group overflow-hidden border-stone-200 hover:shadow-xl transition-all duration-300">
                  <div className="relative aspect-[4/5] overflow-hidden bg-stone-100">
                    ...
                    <div className="absolute top-3 left-3">
                      {daysAgo <= 7 ? (
                        <Badge className="bg-warm-600 text-white shadow-sm border-0 rounded-sm">
                          NEW ARRIVAL
                        </Badge>
                      ) : (
